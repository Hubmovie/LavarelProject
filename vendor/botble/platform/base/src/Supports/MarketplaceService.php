...
class MarketplaceService
{
        ...
        try {
            ...
            if ($response->status() !== 200) {
                ...
                return $this->responseReturn(
                    Arr::get($body, 'message') ?: trans('core/base::marketplace.api_connect_error'),
                    true,
                    [],
            ...
            return $this->responseReturn(trans('core/base::marketplace.api_connect_error'), true);
        }
    }
    ...
    public function beginInstall(string $id, string $type, string $name): JsonResponse
    {
        $core = new Core();
        ...
        return $this->responseReturn(
            trans('core/base::marketplace.install_success'),
            false,
            [
                'name' => $name,
                'id' => $id,
    ...
    protected function extractFile(string $id, string $name): string
    {
        ...
        if (! $zipper->extract($destination, $pathTo)) {
            return $this->responseReturn(trans('core/base::marketplace.unzip_failed'), true);
        }
